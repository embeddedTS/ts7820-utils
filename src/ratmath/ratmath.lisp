...
(defun frac-clk-gen (requested-freq &key (in-freq 125000000) (idiv-bits 7) (frac-bits 11) (chan 0))
  "Calculates a value to send to the Verilog frac_clk_gen core to set a given output frequency.
Second value returned is actual frequency."
  (declare (type real requested-freq in-freq) (type (integer 1 31) idiv-bits frac-bits)
           (optimize speed))
  (let ((n (ratmath:rat (/ in-freq requested-freq) :limd (1- (expt 2 frac-bits)))))
    (multiple-value-bind (idiv frac) (floor n)
      (values
       (dpb chan (byte idiv-bits (+ idiv-bits (* 2 frac-bits)))
            (dpb idiv (byte idiv-bits (* 2 frac-bits))
                 (dpb (numerator frac) (byte frac-bits frac-bits)
                      (dpb (denominator frac) (byte frac-bits 0) 0))))
       (/ in-freq n)))))
